...
for (i in seq_along(vi_list_gt20)) {
  ...
  if (!is.null(res)) {
    ...
    fit_df <- data.frame(DOY = seq(1,365), kNDVI = predict(nlsLM(
      kNDVI ~ res$Intercept_c + res$a1*cos(2*pi*res$Omega*DOY/365) + res$b1*sin(2*pi*res$Omega*DOY/365) +
        res$a2*cos(4*pi*res$Omega*DOY/365) + res$b2*sin(4*pi*res$Omega*DOY/365),
      start = list(
        c=res$Intercept_c, a1=res$a1, b1=res$b1, a2=res$a2, b2=res$b2
      ),
      control=nls.lm.control(maxiter=1) # dummy, only using predict structure
    ), newdata = data.frame(DOY = seq(1,365))))
...
# Replace values < 80 with this mean
deinesharmonicminmaxdf$DOY_min_fit <- ifelse(
  deinesharmonicminmaxdf$DOY_min_fit < 60,
  mean_val,
  deinesharmonicminmaxdf$DOY_min_fit
)
  
plot(result_df$DOY_min_fit, result_df$PDDOY)
plot(result_df$DOY_max_fit, result_df$PDDOY)
plot(result_df$DOY_max_before_min_fit, result_df$PDDOY)
library(ggplot2)
library(ggplot2)

# Output folder
out_dir <- "C:/Users/rbmahbub/Documents/RProjects/DOPDOHYIELD/Figure/DeinesMinMax"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# DOY-Lag pairs
doy_lag_pairs <- list(
  list(doy = "DOY_min_fit", lag = "Laglocalmaxglomax"),
  list(doy = "DOY_max_fit", lag = "Laglocalminglomax"),
  list(doy = "DOY_max_before_min_fit", lag = "Laglocalmaxlocalmin")
)

for (pair in doy_lag_pairs) {
  doy_var <- pair$doy
  lag_var <- pair$lag
  
  # 1. DOY vs PDDOY (color by Lag)
  p1 <- ggplot(result_df, aes_string(x = doy_var, y = "PDDOY", color = lag_var)) +
    geom_point(size = 3) +
    scale_color_viridis_c() +
    theme_minimal(base_size = 14) +
    labs(title = paste(doy_var, "vs PDDOY (color = Lag)"),
         x = doy_var, y = "PDDOY", color = "Lag")
  
  ggsave(file.path(out_dir, paste0(doy_var, "_vs_PDDOY_colorLag.jpeg")),
         p1, width = 6, height = 4, dpi = 300)
  
  # 2. Lag vs PDDOY (color by DOY)
  p2 <- ggplot(result_df, aes_string(x = lag_var, y = "PDDOY", color = doy_var)) +
    geom_point(size = 3) +
    scale_color_viridis_c() +
    theme_minimal(base_size = 14) +
    labs(title = paste(lag_var, "vs PDDOY (color =", doy_var, ")"),
         x = lag_var, y = "PDDOY", color = doy_var)
  
  ggsave(file.path(out_dir, paste0(lag_var, "_vs_PDDOY_color", doy_var, ".jpeg")),
