...
#Shapley vals 
#------------------------------------------------
# SHAP Analysis for Random Forest Phenology Models
# Requires: fastshap, shapr, and ggplot2
...
# fastshap requires a prediction wrapper to understand how to talk to the randomForest object
p_fun <- function(object, newdata) {
  predict(object, newdata = newdata)
}

# 2. SHAP for the Planting Model
# Uses the final_model_planting and the remaining_df_planting from your main script
set.seed(42)

# We take a sample of 100 observations to keep computation time reasonable
shap_sample_p <- remaining_df_planting[sample(nrow(remaining_df_planting), 100), ]
X_planting <- as.matrix(dplyr::select(shap_sample_p, -PDDOY))

# Calculate SHAP values using Monte Carlo integration
# nsim: Number of Monte Carlo repetitions (higher = more accurate but slower)
shap_values_p <- fastshap::explain(
...
# 3. Visualization Function: SHAP Summary Plot
# This replicates the look of the standard SHAP "beeswarm" plots
plot_shap_summary <- function(shap_df, X_df, title) {
  ...
  # Get feature values for coloring (scaled 0 to 1 for visual intensity)
  ...
  # Reorder variables by mean absolute SHAP value (Global Importance)
  var_order <- plot_data %>%
    group_by(Variable) %>%
  ...
  ggplot(plot_data, aes(x = SHAP_Value, y = Variable, color = Feature_Value)) +
    ...
    labs(
      title = title,
      subtitle = "Directional influence on prediction (impact in days)",
      x = "SHAP Value (Contribution to Model Output)",
      y = NULL
    ) +
    theme_minimal(base_size = 14)
}

# 4. Generate Planting SHAP Summary
p_shap_planting <- plot_shap_summary(shap_values_p, X_planting, "Planting Date (PDDOY): SHAP Summary")

# 5. SHAP for the Harvest Model
# Uses final_model_harvest and remaining_df_harvest from your main script
shap_sample_h <- remaining_df_harvest[sample(nrow(remaining_df_harvest), 100), ]
X_harvest <- as.matrix(dplyr::select(shap_sample_h, -HDDOY))

shap_values_h <- fastshap::explain(
  final_model_harvest,
  X = X_harvest,
  pred_wrapper = p_fun,
  nsim = 50,
...
p_shap_harvest <- plot_shap_summary(shap_values_h, X_harvest, "Harvesting Date (HDDOY): SHAP Summary")

# 6. Display and Save Plots
print(p_shap_planting)
print(p_shap_harvest)

# Save high-resolution versions to your working directory
ggsave("planting_shap_summary.jpeg", p_shap_planting, width = 10, height = 7, dpi = 300)
ggsave("harvest_shap_summary.jpeg", p_shap_harvest, width = 10, height = 7, dpi = 300)

# 7. Local Interpretation Example (Single Observation)
# Explaining the specific prediction for the first row of the sample
force_plot_data <- data.frame(
  Variable = colnames(X_planting),
  Contribution = as.numeric(shap_values_p[1, ])
)

p_force <- ggplot(force_plot_data, aes(x = reorder(Variable, Contribution), y = Contribution, fill = Contribution > 0)) +
  geom_col() +
  coord_flip() +
  scale_fill_manual(values = c("TRUE" = "#E64B35FF", "FALSE" = "#4DBBD5FF"), guide = "none") +
  labs(
    title = "Prediction Breakdown: Observation 1 (Planting)",
    subtitle = "How individual features shifted this specific prediction",
    x = "", 
    y = "SHAP Value (Impact in days)"
  ) +
  theme_minimal()
